...
interface TimerDisplayProps { phase: GamePhase; timeRemaining: number; spikeTimer: number; }
...
export const TimerDisplay: React.FC<TimerDisplayProps> = ({ phase, timeRemaining, spikeTimer }) => {
  ...
  const timerColor = isSpikeActive
    ? (isLow ? 'var(--clr-red)' : 'var(--clr-spike)')
    : (isLow ? 'var(--clr-red)' : isMid ? 'var(--clr-spike)' : 'var(--clr-white)');

  const glowColor = isSpikeActive
    ? (isLow ? 'var(--clr-red-glow)' : 'var(--clr-spike-glow)')
    : (isLow ? 'var(--clr-red-glow)' : isMid ? 'var(--clr-spike-glow)' : 'transparent');
  ...
  const ringColor = isSpikeActive
    ? (isLow ? '#e8392a' : '#ff6a00')
    : (isLow ? '#e8392a' : isMid ? '#ff6a00' : '#00d4f0');
    ...
    <div style={{ position:'relative', display:'inline-flex', alignItems:'center', justifyContent:'center', width:'320px', height:'320px' }}>
      ...
      <svg width="320" height="320" style={{ position:'absolute', top:0, left:0, transform:'rotate(-90deg)' }}>
        ...
        {Array.from({length:12},(_,i)=>{
          ...
          const isFilled = (i/12) <= progress;
          const x1=cx+(R-10)*Math.cos(angle), y1=cy+(R-10)*Math.sin(angle);
          const x2=cx+(R+2)*Math.cos(angle),  y2=cy+(R+2)*Math.sin(angle);
          return <line key={i} x1={x1} y1={y1} x2={x2} y2={y2} stroke={isFilled ? ringColor : 'rgba(255,255,255,0.1)'} strokeWidth="2" strokeLinecap="round"/>;
        })}
      </svg>
      ...
      {/* Timer digits */}
      <div style={{
        fontFamily:'var(--font-timer)', fontSize:'clamp(52px,9vw,72px)',
        color: timerColor,
        textShadow:`0 0 20px ${glowColor}, 0 0 40px ${glowColor}`,
        letterSpacing:'6px', zIndex:1, lineHeight:1,
        animation: isLow ? 'timerGlitch 0.8s infinite' : 'none',
      }}>
        {fmt(current)}
      </div>

      {/* "DETONATION" / "ROUND TIME" label inside ring */}
      <div style={{
